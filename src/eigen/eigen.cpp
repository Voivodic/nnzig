...
void eigen_matrixVectorMulAdd(const float* a, const float* b, const float* c, float* result, size_t a_rows, size_t a_cols){
...
void eigen_vectorMatrixMul(float* a, const float* b, size_t b_rows, size_t b_cols){
...
void eigen_vectorMul(const float* a, float* b, size_t size){
...
void eigen_setZero(float* a, size_t size){
    Eigen::Map<Eigen::VectorXf> vecA(a, size);
    vecA.setZero();
}

// Set a matrix to the identity
void eigen_vectorInit(const float* a, float* result, size_t size){
...
void eigen_updateGradWeights(const float* v, const float* y, float* grad, size_t v_size, size_t y_size){
...
void eigen_updateGradBiases(const float* v, float* grad, size_t v_size){
    Eigen::Map<const Eigen::VectorXf> vecV(v, v_size);
